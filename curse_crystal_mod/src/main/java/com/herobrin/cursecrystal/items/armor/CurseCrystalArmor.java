...
public class CurseCrystalArmor extends ArmorItem {
    ...
    public CurseCrystalArmor(Type type, RegistryKey<Item> key) {
        super(CurseArmorMaterial.INSTANCE, type,
            new Settings().registryKey(key).maxCount(1));
    }
    ...
    public void inventoryTick(ItemStack stack, World world, net.minecraft.entity.Entity entity, int slot, boolean selected) {
        ...
        if (hasHelmet && getType() == Type.HELMET)
            world.getEntitiesByClass(LivingEntity.class,
                new Box(player.getPos(),player.getPos()).expand(64), e->e!=player)
        ...
        if (fullSet) {
            ...
            // Ctrl (присесть) без меча = воронка
            if (!world.isClient && player.isSneaking() && getType()==Type.CHESTPLATE) {
                boolean noSword = !(player.getMainHandStack().getItem() instanceof CurseCrystalSword)
                               && !(player.getOffHandStack().getItem() instanceof CurseCrystalSword);
                if (noSword) {
        ...
        } else if (getType()==Type.CHESTPLATE && !player.isCreative() && !player.isSpectator()) {
            ...
            for (int dx=-r;dx<=r;dx+=4)
                for (int dz=-r;dz<=r;dz+=4)
                    if ((long)dx*dx+(long)dz*dz<=(long)r*r) {
                        BlockPos bp=pos.add(dx,-depth,dz);
    ...
    private boolean hasArmor(PlayerEntity player, Item item) {
        for (ItemStack s:player.getArmorItems()) if (s.getItem()==item) return true;
        return false;
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        switch(getType()) {
            ...
            case CHESTPLATE -> tooltip.add(Text.literal("+2 строки HP · Ctrl без меча = Воронка 500 блоков").formatted(Formatting.LIGHT_PURPLE));
